...
namespace js.Service
{
    public class DatabaseConnection
    {
       ...
       public static string sha1(string input)
       {
        ...
        {
            string sql = "CREATE TABLE IF NOT EXISTS Task (id INTEGER PRIMARY KEY AUTOINCREMENT, title VARCHAR(255) NOT NULL, startDate DATE NOT NULL, enddate DATE NOT NULL, toDoListId INT NOT NULL, priority INT NOT NULL, taskFinished boolean, description VARCHAR(1024), FOREIGN KEY(toDoListId) REFERENCES ToDoList(id))";
            SQLiteCommand command = new SQLiteCommand(sql, dbConnection);
            command.ExecuteNonQuery();
        }
        ...
        {
            string sql = "CREATE TABLE IF NOT EXISTS TaskContact (id INTEGER PRIMARY KEY AUTOINCREMENT,  taskId INTEGER NOT NULL, contactId INT NOT NULL, FOREIGN KEY(taskId) REFERENCES Task(id), FOREIGN KEY(contactId) REFERENCES Contact(id))";
            SQLiteCommand command = new SQLiteCommand(sql, dbConnection);
            command.ExecuteNonQuery();
        }
